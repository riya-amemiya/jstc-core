...
export default ({ code, out, conversion }: Props): acorn.OUT => {
    ...
    const Identifier = (code: acorn.Left | acorn.Right): string => {
        if (out.cash.Identifier.findIndex((n) => n.name === code.name) !== -1) {
            if (
                out.cash.Identifier.findIndex((n) => n.to === code.name) === -1
            ) {
    ...
    if (code.test.type === 'BinaryExpression') {
        ...
        if (code.test.left.type === 'Identifier') {
            _argument.BinaryExpression += `${Identifier(code.test.left)}${
                code.test.operator
            }`;
        } else if (code.test.left.type === 'Literal') {
            _argument.BinaryExpression +=
                code.test.left.raw + code.test.operator;
        }
