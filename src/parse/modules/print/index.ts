...
export default ({ code, out, conversion }: Props): acorn.OUT => {
    if (code.expression.type === 'CallExpression') {
        if (code.expression.callee.type == 'Identifier') {
            let has = {
                name: out.ast.Function.findIndex(
                    (n) => n.name === code.expression.callee.name,
                ),
            };
            let comment = '';
            if (has.name === -1) {
                if (
                    ...
                    if (out.not.findIndex((n) => n.num < 1) !== -1) {
                        out = Out.not(out, {
                            name: code.expression.callee.name,
                            num: 0,
                        });
                        console.log(
                            chalk.red(
                                `警告:${code.expression.callee.name}は宣言されていません!`,
                            ),
                        );
                        out.not[
                            out.not.findIndex(
            ...
            }(${out.cash.code.slice(0, -1)})${comment}\n`;
        }
                ...
                if (code.expression.callee.property.name === 'log') {
                    for (const argument of code.expression.arguments) {
                        if (argument.type === 'Literal') {
                            out.cash.code += conversion.Literal(
                                `${argument.raw}`,
                            );
                        }
                        if (argument.type === 'CallExpression') {
                            if (argument.callee.type === 'Identifier') {
                                ...
                                out.cash.code += conversion.FunIdentifier([
                        ...
                        if (argument?.type === 'Identifier') {
                            if (
                                out.cash.Identifier.findIndex(
                                    (n) => n.name === argument.name,
                                ) !== -1
                            ) {
                                if (
                                    out.cash.Identifier.findIndex(
                                        (n) => n.to === argument.name,
                                    ) === -1
                                ) {
                                    out.cash.code += conversion.Identifier(
                                        argument.name.toUpperCase(),
                                    );
                                } else {
                                    out.cash.Identifier.push({
                                        name: `_${argument.name}`,
                                        to: `_${argument.name}`,
