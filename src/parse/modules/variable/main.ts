...
export default (
    ...
    if (code.declarations[0].type === 'VariableDeclarator') {
        ...
        if (code.declarations[0].init.type == 'Identifier') {
            if (
                out.cash.Identifier.findIndex(
                    (n) => n.name === code.declarations[0].init.name,
                ) !== -1
            ) {
                if (
                    out.cash.Identifier.findIndex(
                        (n) => n.to === code.declarations[0].init.name,
                    ) === -1
                ) {
        ...
        if (code.declarations[0].init.type == 'CallExpression') {
            ...
            for (const c of code.declarations[0].init.arguments) {
                if (c.type == 'CallExpression') {
                    if (
                        out.ast.Function.findIndex(
                            (n) => n.name === c.callee.name,
                        ) === -1
                    ) {
        ...
        if (code.kind === 'let' || code.kind === 'var') {
            ...
            out.cash.code += conversion.Kind.let([
                code.declarations[0].id.name,
                raw,
            ]);
        } else if (code.kind === 'const') {
            if (
                out.cash.Identifier.findIndex(
                    (n) => n.name === code.declarations[0].id.name,
                ) === -1
            ) {
                if (
                    out.cash.Identifier.findIndex(
                        (n) =>
                            n.to === code.declarations[0].id.name.toUpperCase(),
                    ) === -1
                ) {
                    varible = [code.declarations[0].id.name.toUpperCase(), raw];
                    out.cash.code += conversion.Kind.const([
                        code.declarations[0].id.name.toUpperCase(),
                        raw,
                    ]);
                    out.cash.Identifier.push({
                        name: code.declarations[0].id.name,
                        to: code.declarations[0].id.name.toUpperCase(),
                        value: code.declarations[0].init.raw,
                        num: 0,
                    });
                } else {
                    varible = [`_${code.declarations[0].id.name}`, raw];
                    out.cash.code += conversion.Kind.const([
                        `_${code.declarations[0].id.name}`,
                        raw,
                    ]);
                    out.cash.Identifier.push({
                        name: code.declarations[0].id.name,
                        to: `_${code.declarations[0].id.name}`,
                        value: code.declarations[0].init.raw,
                        num: 0,
                    });
                }
